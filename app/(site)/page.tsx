...
function Hero() {
  return (
    <section className="relative min-h-screen flex flex-col items-center justify-center px-4 text-center overflow-hidden">
      <BackgroundBeams className="text-cyan-500 dark:text-cyan-400" />
      <div className="relative z-10 space-y-6 max-w-3xl mx-auto w-full">
        ...
        >
          <span className="text-[11px] font-mono text-cyan-600 dark:text-cyan-400 tracking-[0.25em] uppercase opacity-80">
            // Identify
          </span>
        </motion.div>
        ...
        <motion.h1
          ...
          className="text-4xl sm:text-5xl md:text-6xl font-bold text-zinc-900 dark:text-white tracking-tight leading-tight"
        >
        ...
        <div className="pt-2">
          <TerminalHero />
          <noscript>
            <div className="font-mono text-[13px] sm:text-sm w-full max-w-xl mx-auto rounded-lg border border-zinc-300 dark:border-cyan-500/30 bg-white/70 dark:bg-[#0d0d0d]/80 p-4 text-left space-y-2">
              <div>
                <span className="text-emerald-600 dark:text-green-400">lakshyaraj@dev:~$ </span>whoami
                <div className="text-zinc-600 dark:text-zinc-400">Backend Engineer · AI Infrastructure · DevOps</div>
              </div>
              <div>
                <span className="text-emerald-600 dark:text-green-400">lakshyaraj@dev:~$ </span>cat current_focus
                <div className="text-zinc-600 dark:text-zinc-400">Anvil — webhook→BullMQ pipeline</div>
              </div>
              <div>
                <span className="text-emerald-600 dark:text-green-400">lakshyaraj@dev:~$ </span>ls projects/
                <div className="text-zinc-600 dark:text-zinc-400">
                  anchor tripwire goldset quickdraw stackup codecraft homesty anvil
                </div>
              </div>
              <div>
                <span className="text-emerald-600 dark:text-green-400">lakshyaraj@dev:~$ </span>status
                <div className="text-zinc-600 dark:text-zinc-400">Open to backend / AI-infra roles</div>
              </div>
            </div>
          </noscript>
        </div>
        ...
        >
          {[
            ...
            <a
              ...
              rel="noopener"
              className="flex items-center gap-1.5 text-[12px] font-mono text-zinc-500 hover:text-cyan-600 dark:hover:text-cyan-400 transition-colors tracking-wide"
            >
        ...
        <motion.div
          ...
          className="pt-4"
        >
          <div className="inline-flex items-center gap-2 px-3 py-1.5 border border-cyan-500/40 rounded-full text-[11px] font-mono text-cyan-700 dark:text-cyan-400/70">
    ...
    <Section id="now" label="// Mission Status">
      <div className="grid md:grid-cols-2 gap-6">
        <div className="telemetry-card space-y-4">
          ...
          <p className="font-mono text-sm text-zinc-700 dark:text-zinc-200 leading-relaxed">
            {nowData.currently}
          </p>
        </div>
        ...
        <div className="telemetry-card space-y-4">
          ...
          <div className="pt-2">
            ...
            <p className="font-mono text-sm text-zinc-700 dark:text-zinc-300 mt-1">{nowData.location}</p>
          </div>
        </div>
      </div>
    </Section>
...
function ProjectsSection() {
  ...
  const items = sorted.map((project, i) => ({
    id: project.id,
      ...
      <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-4">
        {[
          ...
          <div key={label} className="telemetry-card">
            <p className="font-mono text-[10px] text-zinc-500 dark:text-zinc-600 mb-1 truncate">{label}</p>
            <p className="font-mono text-sm text-cyan-600 dark:text-cyan-400">{value}</p>
          </div>
        ))}
      </div>
    </Section>
...
function Nav() {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 border-b border-zinc-200/70 dark:border-zinc-800/60 bg-white/80 dark:bg-[#050505]/80 backdrop-blur-md">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between">
        <Link href="/" className="font-mono text-sm font-bold text-zinc-800 dark:text-zinc-200 tracking-tight hover:text-cyan-600 dark:hover:text-cyan-400 transition-colors">
          lakshyaraj/
        </Link>
        <div className="flex items-center gap-5 sm:gap-6">
          {links.map(({ href, label }) => (
            <Link
              key={label}
              href={href}
              className="text-[11px] font-mono text-zinc-500 hover:text-cyan-600 dark:hover:text-cyan-400 transition-colors tracking-wide"
            >
              {label}
            </Link>
          ))}
...
export default function HomePage() {
  ...
  // in the server-rendered HTML — crawlers, link unfurls, and no-JS visitors see
  // it. The hero terminal degrades to a <noscript> static transcript.
  return (
    <div className="min-h-screen bg-white dark:bg-[#050505] text-zinc-800 dark:text-zinc-100">
      ...
      <main className="pt-14">
        ...
        <TelemetrySection />
        <ContactSection />
        <footer className="border-t border-zinc-200/70 dark:border-zinc-800/60 py-8 text-center">
